...
x <- SCArray.sat::scNewAssayGDS(fn)
...
```{r}
d
saveRDS(d, "test.rds")  # save to a RDS file

remove(d)  # delete the variable d
gc()       # trigger a garbage collection

d <- readRDS("test.rds")  # load from a RDS file
d
GetAssayData(d, "counts")  # GDS file reopens automatically
```
...
A Seurat object with `SCArrayAssay` can be converted to a Bioconductor `SingleCellExperiment` object using `as.SingleCellExperiment()` in the Seurat package. For example,
