...
fn <- "1M_sc_neurons_d100.gds"    # the input GDS file can be for d250, d500, dfull
d <- scNewSeuratGDS(fn)

d <- NormalizeData(d)
d <- FindVariableFeatures(d, nfeatures=2000)    # (2000 is the default for nfeatures)
d <- ScaleData(d)
...
![**Figure 2**: The benchmark on PCA & UMAP with large datasets (CPU: Intel Xeon Gold 6248 @2.50GHz, RAM: 176GB).](benchmark.svg)
